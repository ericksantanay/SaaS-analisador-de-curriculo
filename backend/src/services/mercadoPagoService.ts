...
export async function mercadoPagoServicePlanoPro(userId: string) {
    ...
    const planoPro = await preference.create({
        body: {
            ...
            external_reference: user.id, // O ID do usuário continua aqui para resgatar no Webhook!
             metadata: {
                plano: "plano_pro"
            },
            notification_url: "https://saas-analisador-de-curriculo.onrender.com/webhook",
            back_urls: {
